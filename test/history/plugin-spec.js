...
describe('Mongoose-History-Trace', async () => {
  ...
  beforeEach(async () => {
    conn = await mongoose.connect('mongodb://localhost:27017/test', dbOptions)
  })
  afterEach(async () => {
  ...
  context('Mongoose OPTIONS', async () => {
    let options = {loggedUser: conn, isAuthenticated: true}
    it.only(`with option ${getPathName(options)}`, test(async ()=> runMethods(options)))
  })
})

async function runMethods (option) {
  context('.CREATE', async () => {
    it.only(`Should call method .create() without ${getPathName(option)} and loggedUser and not Saved History`, test(async () =>{
...
function getPathName(option){
  return (map(option, (value, path) => path)).join(' and ')
}
