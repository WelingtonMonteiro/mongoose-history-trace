 ...
 * @param { mongoose } mongooseConnection - mongoose connection
 * @param { Object } [options]
 * @return {mongoose.Model} History Log Model
 */
module.exports = { HistoryLogModel }

async function HistoryLogModel (mongooseConnection = {}, options = {}) {
  if (!mongooseConnection || isEmpty(mongooseConnection)) {
    console.error('MONGOOSE-HISTORY-TRACE::HistoryLogModel- mongooseConnection options is required.')
    return
  }
  const collectionName = get(options, 'customCollectionName', DEFAULT_COLLECTION_NAME)

  if (!(collectionName in mongooseConnection.models)) {
    return initializeInstanceModel(collectionName, mongooseConnection, options)
  }
 ...
 * @return {*}
 */
async function initializeInstanceModel (collectionName, mongooseConnection, options) {
  // TODO: removed in version v1.8.1 for problem multiples connections and timout exception
  ...
  // }
  return getOrAddNewInstanceModel(mongooseConnection, collectionName, schema)
}
 ...
 */
async function getOrAddNewInstanceModel (mongooseInstance, collectionName, schema) {
...
function addNewPathsInSchema (schema, options) {
  ...
  addCollectionPaths.forEach(({ key, value, defaultValue }) => {
    const newField = {}
    if (TYPES[value]) {
