 ...
 * @return {{conn: any, model}} History Log Model
 */
module.exports.HistoryLogModel = function (options) {
  ...
  const mongooseInstance = get(options, 'mongooseInstance', mongoose)

  if (!(collectionName in mongooseInstance.models)) {
    return initializeInstanceModel(collectionName, options)
  }
  return { model: mongooseInstance.models[collectionName], conn: mongooseInstance}
}
...
function initializeInstanceModel (collectionName, options) {
  ...
  return getOrAddNewInstanceModel(mongooseInstance, collectionName, schema)
}
