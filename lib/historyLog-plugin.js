...
function HistoryPlugin(schema, options = {}) {
     ...
     */
    schema.methods.addLoggedUser = function(loggedUser) {
 ...
 * @param { function } next - function next step
...
async function _saveHistoryLog(object, next, options) {
    if (isEmpty(get(object, 'changes'))) {
        console.error('MONGOOSE-HISTORY-TRACE: path: {changes} no diffs documents.')
        return await next()
    }
...
/**
 * @author Welington Monteiro
 * @param { Object } object - object mapped
 * @param { Object } query -  query to find database
 * @param { String } method - type of operation executed
 * @param { function } next - function next step
 * @return {*}
 */
...
async function _processPreUpdate(next, options) {
    ...
    const pathsSchema = get(this, 'schema.paths')
    ...
    const changes = DeepDiff.getDiff({ old, current, pathsSchema }, options)
...
async function _processPreSave(next, options) {
    ...
    if (this.isNew) changes = DeepDiff.getDiff({ current, pathsSchema }, options)

    if (!this.isNew) changes = DeepDiff.getDiff({ old, current, pathsSchema }, options)
...
async function _processPosRemove(doc, next, options) {
    ...
    const documentNumber = get(old, '_id')
    const changes = DeepDiff.getDiff({ old, pathsSchema }, options)
