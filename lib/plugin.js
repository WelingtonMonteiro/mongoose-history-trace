 ...
 * @example {
 *   mongooseInstance: mongoose conn            //existent connection explicitly
 *   isAbleDisconnect: true                     //default is false, option to disconnect instance of mongoose
...
async function _saveHistoryLog (object, options) {
  const isAbleDisconnect = get(options, 'disconnect') === true
  ...
  const { model, conn } = HistoryLogModel(options)
  const history = new model(object)
  await history.save()
  if (isAbleDisconnect) {
    conn.disconnect()
  }
