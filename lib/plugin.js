...
function HistoryPlugin(schema, options = {}) {
    ...
    async function mountDocToSave({ options, params, doc }, method) {
        ...
        set(
            result,
            "module",
            get(options, "moduleName", get(self, "mongooseCollection.name", get(self, "collection.name")))
        );
    ...
    async function _processCreateHistory(params, options) {
        const { schema, user, module, method, old, current, documentNumber } = await mountDocToSave.call(this, {
            options,
            params,
        });
        const changes = DeepDiff.getDiff(
