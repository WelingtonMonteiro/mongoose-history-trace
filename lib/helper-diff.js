...
const { set, get, map, omit, startCase, camelCase, compact, concat, isEmpty, keys } = require('lodash')
...
class HelperDiff {
  ...
  static getDiff (diffParams = {}, schema = {}, options = {}) {
    ...
    HelperDiff._mapItems(differences, schema, options)
  ...
  static _mapItems (diffs, schema, options) {
    ...
    function _addItem (item) {
      ...
      if (typeof to === 'object') return HelperDiff._objectMapped({ path, item: to, ops, schema }, 'to', options)
      if (typeof from === 'object') return HelperDiff._objectMapped({ path, item: from, ops, schema }, 'from', options)
      const label = HelperDiff._getLabel(path, schema)

      changes.push(HelperDiff._changeTransformToView({ to, path, from, ops, label }, options))
    }
  }
  ...
  static _objectMapped (params = {}, fieldItem, options) {
    ...
    map(item, (value, field) => {
      ...
      if (typeof value === 'object') {
        return HelperDiff._objectMapped({ path: newPath, item: value, ops, schema }, fieldItem, options)
      }
      const change = { from: '', to: '' }
      const label = HelperDiff._getLabel(newPath, schema)

      change[`${fieldItem}`] = value
      changes.push(HelperDiff._changeTransformToView({ ...change, path: newPath, ops, label }, options))
    })
  }
  ...
  static _getLabel (path = '', schema = {}) {
    ...
    const label = HelperDiff._getCustomLabel(paths, schema)
    const nameCapitalized = HelperDiff._capitalizedKey(lastPath)
